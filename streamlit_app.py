...
# Price Levels of Buying and Selling by Top Holders
st.subheader("Price Levels of Buying and Selling by Top Holders")
# Avoid division by zero
top_holders_activity['avg_buy_price_btc'] = top_holders_activity.apply(
    lambda row: row['value_bought_btc'] / row['quantity_bought']
    if row['quantity_bought'] and row['quantity_bought'] != 0 else np.nan, axis=1)
top_holders_activity['avg_sell_price_btc'] = top_holders_activity.apply(
    lambda row: row['value_sold_btc'] / row['quantity_sold']
    if row['quantity_sold'] and row['quantity_sold'] != 0 else np.nan, axis=1)
...
current_df['profitability_ratio'] = current_df.apply(
    lambda row: row['unrealized_profit'] / row['balance'] if row['balance'] != 0 else np.nan, axis=1)
